...
class MockQNTelemetryAggregator:
	...
	func get_current_rtt() -> float: return rtt
	func get_avg_rtt() -> float: return avg_rtt
	func get_current_loss() -> float: return loss
...
func test_deve_retornar_dicionario_com_metricas_formatadas() -> void:
	...
	assert_eq(result["rtt_1_low"], 42.0, "Como é a primeira amostra, o 1% low deve ser igual ao rtt_current")
