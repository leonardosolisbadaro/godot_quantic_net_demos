...
##
## @description
## Ponto de entrada da Demo 02. Inicializa o Dashboard de telemetria
## e sincronização de relógio, delegando medições ao adaptador.
##
## @created 2026-08-07
## @updated 2026-08-08
##
...
func _on_state_changed(state_name: String, is_connected: bool) -> void:
	if not is_instance_valid(_lbl_state):
		return
...
func _on_metrics_updated(data: Dictionary) -> void:
	if not is_instance_valid(_lbl_rtt_current):
		return
