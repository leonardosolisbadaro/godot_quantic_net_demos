...
const HandleConnectionStateChangeUseCase = preload("res://src/use_cases/handle_connection_state_change_use_case.gd")
...
func test_deve_registrar_falha_quando_recebe_status_failed() -> void:
	...
	assert_eq(_session.current_state, NetworkSession.State.FAILED, "Sessão deve registrar a falha de handshake")
...
func test_deve_limpar_sessao_quando_desconecta() -> void:
	...
	assert_eq(_session.current_state, NetworkSession.State.DISCONNECTED, "Sessão deve voltar ao estágio inicial")
