...
func test_estado_inicial_deve_ser_disconnected() -> void:
	...
	assert_eq(
		...
		"Sessão recém criada deve estar desconectada",
	)
...
func test_iniciar_conexao_deve_mudar_estado_para_connecting() -> void:
	...
	assert_eq(
		...
		"Ao chamar start_connection, o estado deve transitar para CONNECTING",
	)
...
func test_nao_deve_permitir_conectar_se_ja_estiver_conectado() -> void:
	...
	assert_eq(
		...
		"[Pre-Condição] O estado atual deve ser CONNECTED",
	)
	...
	assert_eq(
		_sut.current_state,
		NetworkSession.State.CONNECTED,
		"O estado deve permanecer intacto (CONNECTED)",
	)
