...
func before_each() -> void:
	...
	_adapter = EntityRegistryAdapter.new(
		_mock_gateway,
		_mock_spawn_use_case,
		_mock_despawn_use_case,
		_mock_factory,
	)
...
func test_deve_chamar_spawn_use_case_quando_peer_entra_como_servidor() -> void:
	# Arrange
	_mock_gateway.mock_is_server = true
...
func test_nao_deve_chamar_spawn_use_case_quando_peer_entra_como_cliente() -> void:
	# Arrange
	_mock_gateway.mock_is_server = false
...
class MockGateway:
	...
	var mock_is_server: bool = false


	func is_server() -> bool:
		return mock_is_server
