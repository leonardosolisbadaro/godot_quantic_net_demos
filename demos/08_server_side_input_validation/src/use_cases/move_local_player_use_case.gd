...
func execute(current_pos: Vector3, input_dir: Vector3, speed: float, delta: float, inject_teleport: bool = false, submit_network: bool = true) -> Vector3:
	...
	if submit_network:
		...
		# Usamos o parâmetro `rot` (rotação) que estava vazio para empacotar o `input_dir` 
		# para a nova validação Input-Based do servidor!
		_gateway.submit_state(predicted_pos, Vector3(input_dir.x, 0, input_dir.z), 0, delta)
