...
func execute(entity_id: int, current_visual_pos: Vector3, weight: float) -> Vector3:
	...
	# Retornamos o Lerp agressivo para corrigir Buffer Underruns criados pelo NETEM
	# e garantir suavidade (60+ FPS) em cima de estados discretos recebidos (20Hz).
	return current_visual_pos.lerp(target_pos, weight)
